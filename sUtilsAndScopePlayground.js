...
1. var → Hoisted & initialized as undefined → safe to access (but gives undefined)
2. let & const → Hoisted but in TDZ → cannot access before declaration
3. Function declarations → fully hoisted → safe to call anywhere
4. Function expressions (var) → variable is hoisted but not the assignment → undefined until assignment
5. Arrow functions (let) → behave like let variables → TDZ applies
*/
